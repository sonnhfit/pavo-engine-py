...
def init_sequence(file_path=None, json_data=None):
    if file_path != None:
        json_data = read_json_video(file_path)
    strips = get_strips_from_json(json_data)
    seq = Sequence(strips=strips, n_frame=json_data['timeline']['n_frames'])
    return seq


def render(input_file_path=None, json_data=None):
    seq = init_sequence(input_file_path, json_data)
